                ...
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <Button
                      key={cat.id}
                      variant="ghost"
                      onClick={() => router.push(cat.id === "all" ? "/produk" : `/produk?category=${cat.id}`)}
                      className={cn(
                        "rounded-full px-6 h-10 text-sm font-medium transition-all border",
                        isActive
                          ? "bg-[#1F3D2B] text-white border-[#1F3D2B] hover:bg-[#1F3D2B] hover:text-white" // Active
                          : "bg-white text-[#1C1C1C] border-[#E6E3DF] hover:border-[#1F3D2B] hover:text-[#1F3D2B]" // Default
                      )}
                    >
                      {cat.label}
                    </Button>
